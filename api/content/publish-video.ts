...
export default async function handler(req: any, res: any) {
  // Top-level try/catch guarantees the response is always JSON. Without
  // this, an uncaught throw (e.g. missing env var) becomes Vercel's
  // default HTML 500 page and the editor's `await res.json()` blows up
  // with a confusing "Unexpected token 'A'" error.
  try {
    ...
    // Explicit env var check so the error message is actionable.
    const missing = ['ONEUP_API_KEY', 'ONEUP_CATEGORY_ID', 'ONEUP_YOUTUBE_CHANNEL_ID']
      .filter((k) => !process.env[k])
    if (missing.length) {
      return res.status(500).json({
        error: `Server missing required env vars: ${missing.join(', ')}. Add them in Vercel → Settings → Environment Variables, then redeploy.`,
      })
    }
